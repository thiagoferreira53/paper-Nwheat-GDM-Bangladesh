...
```{r merging observed and simulated data}
...
dt_sim$EDAPS <- as.Date(as.character(dt_sim$EDAT), format = "%Y%j") - 
  as.Date(as.character(dt_sim$PDAT), format = "%Y%j")

dt_sim$ADAPS <- as.Date(as.character(dt_sim$ADAT), format = "%Y%j") - 
  as.Date(as.character(dt_sim$PDAT), format = "%Y%j")

dt_sim$MDAPS <- as.Date(as.character(dt_sim$MDAT), format = "%Y%j") - 
  as.Date(as.character(dt_sim$PDAT), format = "%Y%j")
...
```{r Functions to calculate metrics}
...
calculate_metrics <- function(observed, simulated) {
  ...
  d_stat <- 1 - (sum((simulated - observed)^2) / sum((abs(simulated - mean(observed)) + abs(observed - mean(observed)))^2))
  nse <- 1 - (sum((simulated - observed)^2) / sum((observed - mean(observed))^2))
  return(c(RMSE = rmse, D_Stat = d_stat, NSE = nse))
}

metrics <- list(
  Yield = calculate_metrics(merged_df$HWAMM, merged_df$HWAM),
  Anthesis = calculate_metrics(merged_df$ADAPM, merged_df$ADAPS),
  Maturity = calculate_metrics(merged_df$MDAPM, merged_df$MDAPS),
  Grain_Number = calculate_metrics(merged_df$HIAMM, merged_df$HIAM * 100),
  Biomass = calculate_metrics(merged_df$CWAMM, merged_df$CWAM)
)

format_metrics <- function(metrics) {
  sprintf("RMSE: %.2f\nD-stat: %.2f\nNSE: %.2f", metrics["RMSE"], metrics["D_Stat"], metrics["NSE"])
}

yield_metrics <- format_metrics(metrics$Yield)
anthesis_metrics <- format_metrics(metrics$Anthesis)
maturity_metrics <- format_metrics(metrics$Maturity)
grain_number_metrics <- format_metrics(metrics$Grain_Number)
biomass_metrics <- format_metrics(metrics$Biomass)
...
# 1:1 plot for yield
c1 <- ggplot(merged_df, aes(x = HWAMM, y = HWAM, color = sowing, label = TNAM)) +  
  geom_point() +
  ...
  xlim(800, 7000) +
  ylim(800, 7000) +
  annotate("text", x = 1000, y = 6150, label = yield_metrics, hjust = 0)
c1

# 1:1 plot for anthesis
c2 <- ggplot(merged_df, aes(x = ADAPM, y = ADAPS, color = sowing, label = TNAM)) +  
  geom_point() +
  geom_abline() +
  ...
  xlim(40, 120) +
  ylim(40, 120) +
  annotate("text", x = 50, y = 105, label = anthesis_metrics, hjust = 0)
c2

# 1:1 plot for maturity
c3 <- ggplot(merged_df, aes(x = MDAPM, y = MDAPS, color = sowing, label = TNAM)) +  
  geom_point() +
  geom_abline() +
  ...
  xlim(80, 170) +
  ylim(80, 170) +
  annotate("text", x = 90, y = 155, label = maturity_metrics, hjust = 0)
c3

# 1:1 plot for grain number
c4 <- ggplot(merged_df, aes(x = HIAMM, y = HIAM * 100, color = sowing, label = TNAM)) +  
  geom_point() +
  geom_abline() +
  ...
  xlim(20, 60) +
  ylim(20, 60) +
  annotate("text", x = 25, y = 53, label = grain_number_metrics, hjust = 0)
c4

# 1:1 plot for biomass
c5 <- ggplot(merged_df, aes(x = CWAMM, y = CWAM, color = sowing, label = TNAM)) +  
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  ...
  xlim(6000, 15000) +
  ylim(6000, 15000) +
  annotate("text", x = 6500, y = 13500, label = biomass_metrics, hjust = 0)
c5
...
Choudhury, A. K., Hena, M. A.,  , S., Choudhury, A. K., Ishtiaque, S., Sen, R., Jahan, M. A. H. S., Akhter, S., Ahmed, F., Biswas, J. C., Manirruzaman, M., Hossain, M. B., Miah, M. M., Rahman, M. M., Zahan, T., Khan, A. S. M. M. R., & Kalra, N. (2018). Calibration and Validation of DSSAT Model for Simulating Wheat Yield in Bangladesh Haya: The Saudi Journal of Life Sciences (SJLS) Calibration and Validation of DSSAT Model for Simulating Wheat Yield in Bangladesh. https://doi.org/10.21276/haya.2018.3.4.3 
